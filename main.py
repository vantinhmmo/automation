...
if __name__ == "__main__":
    x = requests.get('https://dash.minproxy.vn/api/rotating/v1/proxy/get-new-proxy?api_key=txbC12bh4kqo9ftUkqV5FJYwXcxhjC0k')
    nx = x.json()
    PROXY = nx['data']['http_proxy']
    print(PROXY)
    print(nx['data']['username'])

    driver = createBrowser(PROXY.split(':')[0], int(PROXY.split(':')[1]), nx['data']['username'], nx['data']['password'])
    sleep(30000)
